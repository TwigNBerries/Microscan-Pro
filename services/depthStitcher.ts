...
export function serializeToBlob(
  ...
  for (let gy = 0; gy < globalH; gy++) {
    ...
    for (let gx = 0; gx < globalW; gx++) {
      ...
      const gi = gy * globalW + gx;
      const w = wAccum[gi];
      // Use -1 µm as sentinel for uncovered cells — no real depth pixel is negative,
      // and Gwyddion chokes on literal "NaN". Mask in Gwyddion via Data Process →
      // Mask → Mask by Threshold with upper bound < 0.
      const zStr = w > 0 ? (zAccum[gi] / w).toFixed(4) : "-1.0000";
