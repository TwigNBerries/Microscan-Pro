...
def main():
    try:
        ...
        log(f"Generating bare heatmap tile (vmin={vmin:.3f}, vmax={vmax:.3f})...")
        # Save a decoration-free heatmap: pure turbo colormap pixels, no axes/colorbar/labels.
        # Tiles must be clean so they can be mosaic-stitched; decorations are added by the
        # JS frontend only on the final stitched image.
        try:
            cmap_func = plt.get_cmap('turbo')
            normalized = np.clip((height - vmin) / (vmax - vmin), 0, 1)
            nan_mask = ~np.isfinite(height)
            normalized[nan_mask] = 0.0
            rgba = (cmap_func(normalized) * 255).astype(np.uint8)
            rgb = rgba[:, :, :3]
            output_image_path = os.path.join(image_folder, 'heatmap.jpg')
            Image.fromarray(rgb).save(output_image_path, quality=95)
            log("Bare heatmap tile saved.")
        except Exception as e:
            log(f"ERROR during heatmap generation: {str(e)}")
            try:
